        ...
        if (kbhit()) {
            ...
            if (tecla == 27) break;
                ...
                if (num_linea < total_lineas) {
                    ...
                    freopen("inventario.txt", "w", archivo);
...
void dibujarInterfazVendedor() {
    ...
    while(true) {
        ...
        if(kbhit()) {
            ...
            switch(tecla) {
                case '1': mostrarListaArchivo("Lista Completa", "inventario.txt"); debe_redibujar = true; break;
                case '2': mostrarListaArchivo("Por Tipo de Vehiculo", "llantas_por_tipo.txt"); debe_redibujar = true; break;
                case '3': mostrarListaArchivo("Por Tipo de Uso", "llantas_por_uso.txt"); debe_redibujar = true; break;
                case '4': mostrarListaArchivo("Llantas Agotadas", "llantas_agotadas.txt"); debe_redibujar = true; break;
            }
        }
        delay(20);
    }
...
void dibujarInterfazAlmacenista() {
    ...
    while(true) {
        if(debe_redibujar) {
            ...
            dibujarBoton(100, 150, 400, 40, "1. Mostrar Inventario");
        ...
        if(kbhit()) {
            ...
            switch(tecla) {
                ...
                case '2': {
                    // Agregar producto con 4 campos: Nombre | Cantidad | Medidas | Tipo de Uso
                    limpiarPantalla();
                    ...
                    // Función para pedir texto en pantalla con edición básica
                    auto pedirTexto = [](int y, const char* prompt, char* buffer, int max_len) {
                        setcolor(COLOR_TEXTO_NORMAL);
                        outtextxy(50, y, (char*)prompt);
                        int idx = 0;
                        buffer[0] = '\0';
                        while (true) {
                            if (kbhit()) {
                                ...
                                if (c == 13) break;
                                    ...
                                    buffer[idx] = '\0';
                                    setfillstyle(SOLID_FILL, COLOR_FONDO);
                                    bar(50, y + 20, 700, y + 40);
                                    outtextxy(50, y + 20, buffer);
                    ...
                    pedirTexto(150, "Ingrese nombre del producto:", nombre, sizeof(nombre));
                    pedirTexto(200, "Ingrese cantidad:", cantidad, sizeof(cantidad));
                    pedirTexto(250, "Ingrese medidas:", medidas, sizeof(medidas));
                    pedirTexto(300, "Ingrese tipo de uso:", tipo_uso, sizeof(tipo_uso));

                    if (strlen(nombre) > 0 && strlen(cantidad) > 0) {
                        FILE* f = fopen("inventario.txt", "a");
                        if (f) {
                            char linea[MAX_LINEA_LONG];
                            snprintf(linea, sizeof(linea), "%s | %s | %s | %s\n", nombre, cantidad, medidas, tipo_uso);
                            fputs(linea, f);
                            fclose(f);
                            setcolor(COLOR_EXITO);
                            outtextxy(50, 350, (char*)"Producto agregado correctamente.");
                            Sleep(1500);
                        } else {
                            setcolor(COLOR_ADVERTENCIA);
                            outtextxy(50, 350, (char*)"No se pudo abrir inventario.txt");
                            Sleep(1500);
                        }
                    }
                    debe_redibujar = true;
...
void dibujarInterfazGerente() {
    ...
    while(true) {
        if(debe_redibujar) {
            ...
            dibujarBoton(100, 150, 400, 40, "1. Ganancias");
